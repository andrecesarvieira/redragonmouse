...
podman run --rm \
    ...
    bash -euxo pipefail -c '
        ...
        mkdir -p /tmp/redragon-control-0.1.0
        cp -a /src/redragon_control /src/tests /src/packaging /src/run.py \
            /src/README.md /tmp/redragon-control-0.1.0/
        tar -C /tmp -czf /tmp/rpmbuild/SOURCES/redragon-control-0.1.0.tar.gz \
            redragon-control-0.1.0
